...
void poisson_solver(const IplImage *img, IplImage *gxx , IplImage *gyy, Mat &result){
	...
	for(int i =1;i<h-1;i++)
		for(int j=1;j<w-1;j++){
			idx=i*w + j;
			f_bp[idx] = -4*(int)bound.at<uchar>(i,j)
						+(int)bound.at<uchar>(i,(j+1))
						+(int)bound.at<uchar>(i,(j-1))
...
IplImage *poisson_blend(IplImage *I, IplImage *mask, int posx, int posy){
	...
	int wMask = mask->width;
	int hMask = mask->height;
	int channel1 = mask->nChannels;

	//get gradiant map of both I and mask
	getGradientx(I,grx);
	getGradienty(I,gry);
	...
	//create a mask map which white pixels is where mask in I
	for(int i=posx, ii =0;i<posx+wMask;i++,ii++)
		for(int j=0,jj=posy;j<hMask;j++,jj++)
			for(int c=0;c<channel;++c){
				CV_IMAGE_ELEM(S,uchar,jj,i*channel+c) = 255;
			}
	...
	//copy gradiant map of mask into mask map S
	for(int i=posx, ii =0;i<posx+wMask;i++,ii++)
		for(int j=0,jj=posy;j<hMask;j++,jj++)
			for(int c=0;c<channel;++c){
				CV_IMAGE_ELEM(bmaskx,float,jj,i*channel+c) = CV_IMAGE_ELEM(sgx,float,j,ii*channel+c);
				CV_IMAGE_ELEM(bmasky,float,jj,i*channel+c) = CV_IMAGE_ELEM(sgy,float,j,ii*channel+c);
			}
	...
	//create a new gradient map, with I's gradient and mask's gradient in ROI
	for(int i=0;i < h; i++)
		for(int j=0; j < w; j++)
			for(int c=0;c<channel;++c){
